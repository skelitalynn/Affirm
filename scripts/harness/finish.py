...
def main() -> int:
    ...
    git_status = git.stdout.strip() if git.returncode == 0 else ""
    if git.returncode == 0:
        changed_count = len([line for line in git_status.splitlines() if line.strip()])
        git_summary = "clean" if changed_count == 0 else f"dirty ({changed_count} path(s) changed)"
    else:
